...
public class UpdateServletNews extends HttpServlet {
       
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		response.setContentType("text/html; charset=utf-8");
		Logger logger=Logger.getLogger(UpdateServletNews.class.getName());
			...
			while (iterator.hasNext()) {
				...
				if (item.isFormField()) {
					    if(item.getFieldName().equals("nid")){
					        n.setNid(Integer.parseInt(item.getString("UTF-8")));
		...
		int flag = ns.updateNews(n);
		if (flag == 0) {
			pw.write("<script type=\"text/javascript\">");
			pw.write("alert(\"新闻主题修改失败,请重新输入！\");");
			pw.write("location.href=\"" + contextPath + "/newspages/admin.jsp\";");
			pw.write("</script>");
			logger.warn("新闻主题修改失败！");
		} else {
			pw.write("<script type=\"text/javascript\">");
